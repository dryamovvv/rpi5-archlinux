...
Этот репозиторий собирает образ Arch Linux для Raspberry Pi 5 с помощью Bash-скриптов. `scripts/main.sh` — единственный поддерживаемый CLI entrypoint. Framework-код вынесен в `lib/core/`, build-модули — в `lib/modules/`, низкоуровневая логика — в `lib/` (`disk.sh`, `bootstrap.sh`, `log.sh`, `colors.sh`). Каталог `conf/` хранит активные статические ресурсы сборки; reference-шаблоны вынесены в `conf/reference/`. В текущем build path реально используются `conf/build.conf` и `conf/pacman-arm.conf`, а `cmdline.txt` и `config.txt` сейчас генерируются из `lib/bootstrap.sh`. Артефакты сборки, такие как `arch_root.img`, не должны попадать в коммиты.

## Build, Test, and Development Commands
Запускайте основной сценарий командой `sudo ./scripts/main.sh build`; он создает и настраивает `arch_root.img`. Для инспекции используйте `./scripts/main.sh list-steps`, для проверки конфигурации — `./scripts/main.sh validate`. Перед коммитом выполняйте `bash -n scripts/*.sh lib/*.sh lib/core/*.sh lib/modules/*.sh tests/*.sh` для быстрой проверки синтаксиса всех поддерживаемых shell-файлов. Если установлен `shellcheck`, запускайте `shellcheck scripts/*.sh lib/*.sh lib/core/*.sh lib/modules/*.sh tests/*.sh`, чтобы поймать проблемы с quoting, `source` и переносимостью. Релизный workflow публикует `arch_root.img.xz`, а не сырой `.img`, из-за лимита GitHub Release на размер asset, и собирает образ напрямую на native `ubuntu-24.04-arm` runner без собственного builder-контейнера. Проверять итоговый образ удобно стандартными системными утилитами, например `losetup -fP arch_root.img` и `mount`.
...
## Testing Guidelines
Формального тестового фреймворка пока нет, поэтому проверка сейчас в основном shell-based и ручная. Минимум — прогонять `bash -n`, запускать smoke-проверки из `tests/`, запускать `./scripts/main.sh validate` и валидировать workflow YAML. Проверять измененный build path нужно через `sudo ./scripts/main.sh build` или соответствующий GitHub Actions workflow. Build-time настройка должна по возможности избегать `arch-chroot`; предпочтительны `systemd-firstboot`, прямые file writes и first-boot service. При изменении `conf/` сначала проверьте, подключен ли файл к реальному build path; не предполагается, что любой файл из `conf/` автоматически попадает в образ. Ручную валидацию фиксируйте в описании PR.
